...
const updateUsername = async (req: Request, res: Response): Promise<void> => {
  const existingUser = await getOneByField(User, 'email', (req as IRequest).userToken.email);
  const response = await updateOne(User, existingUser?._id as string, { username: req.body.username });
